...
//            //*** подписываю
//            str = tr("%1σ").arg(md(values_q[i].value));
//            QGraphicsTextItem *text_rod = new QGraphicsTextItem(str);
...
//            //*** подписываю
//            str = tr("%1F").arg(md(values_F[i].value));
//            QGraphicsTextItem *text_rod = new QGraphicsTextItem(str);
...
////    QPixmap pix(":/image/pencil2.png");

////    QPixmap pix_sized = pix.scaled(QSize(50, 50), Qt::KeepAspectRatioByExpanding);
////    QPixmap pix_sized = pix.scaled(QSize(50, 50), Qt::KeepAspectRatioByExpanding);
...
////    if (start_x_rod.size()) {
////        new_x = start_x_rod[0] + sdvig_x;
////        new_y = start_y_rod[0];
////    }

////    QPixmap pix_sized = QPixmap(pix.copy(0,0,
////           (ui->graphicsView->width() - new_x - 30*4),
////           (ui->graphicsView->height() - new_y - 30*4)));
////    QGraphicsPixmapItem *pix_item =
////            scene->addPixmap(pix_sized);
...
////    if (start_x_rod.size())
////        pix_item->setPos(new_x, new_y);

//    if (add_h != 0) {
//        add_elem_type(start_x_rod, real_size_w_rod, add_h,
//                      "Nx", size_y/2. + maxh/2.*ky + above*5./10. + (add_h - 29)*1./3.,
//                      1);
//        add_elem_type(start_x_rod, real_size_w_rod, add_h,
//                      "Fx", size_y/2. + maxh/2.*ky + above*5./10. + (add_h - 29)*2./3. - 15,
//                      2);
//        add_elem_type(start_x_rod, real_size_w_rod, add_h,
//                      "σx", size_y/2. + maxh/2.*ky + above*5./10. + (add_h - 29)*3./3. - 30 - 1,
//                      3);
//    }
...
//    QString path = "C:/Qt/4lab1/qtgraphcs/chekanin/111/111/QGraphicsFramework/";
//    QImage image_copy = image;
////    if (image.height() > 800)
////    image_copy = image.scaled(image.width()/1.5, image.height()/1.5, Qt::KeepAspectRatio);
//    image_copy.save(path + "result.png");
////    qDebug() << ui->graphicsView->verticalScrollBar()->value();
////    ui->graphicsView->setVerticalScrollBar(0);
...
//                    int add_h, QString name, int center,
//                    int type)
//{
//    QString str = name;
//    QFontMetrics f(str);
...
//    text->setPos(sdvig_x - f.width(str) - 10, center - f.height()/2. - 2);
//    scene->addItem(text);
...
//        for (double step = start_line_x; step <= end_line_x; step += 0.003) {
...
//        if (v_x[i] == 0) {
//            if ((md((*hei)[i], 2) != 0)
//                    && (md(md((*hei)[i], 2)) != md(md(a_nujno_li[1], 2)))) {
...
//                a_nujno_li[0] = (*hei)[i];
//                //***
//                str = tr("%1").arg(md(md(fabs((*hei)[i]), 4))) + "qL";
//                //***
////                a_nujno_li[0] = md(fabs((*hei)[i]), 4);
//                //***
//                QFontMetrics f(str);
//                QGraphicsTextItem *text = new QGraphicsTextItem(str);
...
//                    && (md(md((*hei)[i + (i == v_x.size() - 2 ? 1 : 0)], 2)) != md(md(a_nujno_li[0], 2)))
//                    /*&& (md(md((*hei)[i + (i == v_x.size() - 2 ? 1 : 0)], 2)) != md(md(a_nujno_li[1], 2)))*/) {
...
//                               mas[2], mas[3]);
//                //***
//                a_nujno_li[1] = (*hei)[i + (i == v_x.size() - 2 ? 1 : 0)];
//                //***
//                str = tr("%1").arg(md(md(fabs((*hei)[i + (i == v_x.size() - 2 ? 1 : 0)]), 4))) + "qL";
//                //***
////                a_nujno_li[1] = md(fabs((*hei)[i + (i == v_x.size() - 2 ? 1 : 0)]), 4);
//                //***
//                QFontMetrics f(str);
//                QGraphicsTextItem *text = new QGraphicsTextItem(str);
